            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="recipient_name" value="{{ old('recipient_name') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('recipient_name') border-red-500 @enderror"
                        required>
                ...
                <div>
                    ...
                    <input type="text" name="purpose" value="{{ old('purpose') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('purpose') border-red-500 @enderror">
                    @error('purpose')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
    ...
    @push('scripts')
        <script>
            ...
            function addItem() {
                ...
                row.innerHTML = `
        <td class="px-4 py-3">
            <select name="items[${itemIndex}][item_id]" class="w-full px-3 py-2 border border-gray-300 rounded" required onchange="checkStock(${itemIndex}, ${warehouseId})">
            ...
            async function checkStock(index, warehouseId) {
                ...
                try {
                    const response = await fetch(
                        `{{ route('outgoing-goods.stock.check') }}?warehouse_id=${warehouseId}&item_id=${itemId}`);
