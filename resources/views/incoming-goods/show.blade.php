    ...
    <div class="bg-white rounded-lg shadow-md">
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="flex space-x-2">
                    @if ($incomingGood->status == 'pending')
                        <form action="{{ route('incoming-goods.approve', $incomingGood) }}" method="POST" class="inline">
                            @csrf
                        ...
                        <form action="{{ route('incoming-goods.reject', $incomingGood) }}" method="POST" class="inline">
                            @csrf
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <dl class="space-y-2">
                        <div class="flex justify-between">
                            ...
                            <dd class="font-semibold">{{ $incomingGood->transaction_code }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-600">Tanggal:</dt>
                            <dd class="font-semibold">{{ $incomingGood->transaction_date->format('d/m/Y') }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-600">No. Invoice:</dt>
                            <dd class="font-semibold">{{ $incomingGood->invoice_number ?? '-' }}</dd>
                        </div>
                        <div class="flex justify-between">
                            ...
                            <dd>
                                ...
                                @elseif($incomingGood->status == 'approved')
                                    <span
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-semibold text-gray-700 mb-3">Informasi Lainnya</h3>
                    <dl class="space-y-2">
                        <div class="flex justify-between">
                            <dt class="text-gray-600">Warehouse:</dt>
                            <dd class="font-semibold">{{ $incomingGood->warehouse->name }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-600">Supplier:</dt>
                            <dd class="font-semibold">{{ $incomingGood->supplier->name }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-600">Dibuat Oleh:</dt>
                            <dd class="font-semibold">{{ $incomingGood->user->name }}</dd>
                        </div>
                        @if ($incomingGood->approved_by)
                            <div class="flex justify-between">
                                <dt class="text-gray-600">Diproses Oleh:</dt>
                                <dd class="font-semibold">{{ $incomingGood->approver->name }}</dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="text-gray-600">Tanggal Proses:</dt>
                                <dd class="font-semibold">{{ $incomingGood->approved_at->format('d/m/Y H:i') }}</dd>
                            </div>
                        @endif
                    </dl>
                </div>
            ...
            @if ($incomingGood->notes)
                <div class="bg-blue-50 p-4 rounded-lg mb-6">
                    ...
                    <p class="text-gray-700">{{ $incomingGood->notes }}</p>
                </div>
            @endif
